...
function handleInstagramUrl($chatId, $url) {
    try {
        ...
        if (file_exists($filename) && filesize($filename) > 0) {
            ...
            file_put_contents('error.log', date('Y-m-d H:i:s') . " - Download failed, file not created: " . $filename . "\n", FILE_APPEND);
        }
        
    } catch (Exception $e) {
        sendMessage($chatId, "⚠️ An error occurred while processing your request.");
